...
export const POST: RequestHandler = async (event) => {
    assertAuth(event);
    const { request } = event;
    const { name, description, guidelines, bannerImage, icon, publishNow }: CreateChannel =
        await request.json();
    ...
    if (publishNow) {
        ...
        if (existingChannel) {
            return json({ name: 'Channel name already in use' }, { status: 409 });
        }
    } else {
        const existingChannel = await getPrivateChannel(db, event.locals.user.id, name);
        if (existingChannel) {
            return json({ name: 'You already have a channel by this name' }, { status: 409 });
        }
    }

    {
        ...
        if (description && description.length > 4000)
            err.description = 'Description must be fewer than 4000 characters';
        if (guidelines && guidelines.length > 4000)
            err.guidelines = 'Guidelines must be fewer than 4000 characters';
